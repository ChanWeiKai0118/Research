...
def save_to_gsheet(data, sheet_name):
    ...
    if sheet_name == "chemo_data":
        ...
        row[58] = f'=IF(BI{last_row}="","",TO_DATE(XLOOKUP(BI{last_row}, FILTER(lab_data!H:H, (lab_data!A:A = B{last_row}) * (lab_data!E:E > F{last_row}) * (lab_data!E:E <= F{last_row} + 89)), FILTER(lab_data!E:E, (lab_data!A:A = B{last_row}) * (lab_data!E:E > F{last_row}) * (lab_data!E:E <= F{last_row} + 89)), "Not found")))'
        row[59] = f'=IF(BI{last_row}="", "", IF(D{last_row}=0, IF(BI{last_row}<=0.7, 141*((BI{last_row}/0.7)^-0.329)*0.993^I{last_row}*1.018, 141*((BI{last_row}/0.7)^-1.209)*0.993^I{last_row}*1.018), IF(BI{last_row}<=0.9, 141*((BI{last_row}/0.9)^-0.411)*0.993^I{last_row}, 141*((BI{last_row}/0.9)^-1.209)*0.993^I{last_row})))'
        ...
        row[61] = f'=IF(BF{last_row}="","",TO_DATE(XLOOKUP(BF{last_row}, FILTER(lab_data!H:H, (lab_data!A:A = B{last_row}) * (lab_data!E:E > F{last_row}) * (lab_data!E:E <= F{last_row} + 14)), FILTER(lab_data!E:E, (lab_data!A:A = B{last_row}) * (lab_data!E:E > F{last_row}) * (lab_data!E:E <= F{last_row} + 14)), "Not found")))'
        row[62] = f'=IF(BF{last_row}="", "", IF(D{last_row}=0, IF(BF{last_row}<=0.7, 141*((BF{last_row}/0.7)^-0.329)*0.993^I{last_row}*1.018, 141*((BF{last_row}/0.7)^-1.209)*0.993^I{last_row}*1.018), IF(BF{last_row}<=0.9, 141*((BF{last_row}/0.9)^-0.411)*0.993^I{last_row}, 141*((BF{last_row}/0.9)^-1.209)*0.993^I{last_row})))'
        
        # AKI, AKD判定
        row[64] = f'=IF(BI{last_row}="", 0, IF(D{last_row}=1,IF(Q{last_row}>=1.3,IF(OR(BI{last_row}/P{last_row}>=1.5, BI{last_row}/Q{last_row}>=1.5), 1, 0),IF(OR(BI{last_row}/P{last_row}>=1.5, BI{last_row}/Q{last_row}>=1.5, BI{last_row}/1.3>=1.5), 1, 0)),IF(Q{last_row}>=1.1,IF(OR(BI{last_row}/P{last_row}>=1.5, BI{last_row}/Q{last_row}>=1.5), 1, 0),IF(OR(BI{last_row}/P{last_row}>=1.5, BI{last_row}/Q{last_row}>=1.5, BI{last_row}/1.1>=1.5), 1, 0))))'
        row[65] = f'=IF(BF{last_row}="", 0, IF(U{last_row}<60, IF(OR(BF{last_row}/Q{last_row}>=1.5,BF{last_row}/P{last_row}>=1.5, BH{last_row}/U{last_row}<0.65,BH{last_row}/T{last_row}<0.65, BM{last_row}=1), 1, 0), IF(OR(BF{last_row}/Q{last_row}>=1.5,BF{last_row}/P{last_row}>=1.5, BH{last_row}/U{last_row}<0.65,BH{last_row}/T{last_row}<0.65, BM{last_row}=1, BH{last_row}<60), 1, 0)))'
        ...
        if input_number and input_date_str:
            try:
                ...
                if selected_row.empty:
                    ...
                    # ----SHAP individual waterfall plot----
                    # 將 shap_values 轉成 np.array shape=(20,)
                    shap_vals_AKI = np.array(shap_values_AKI).flatten()
                    plot_waterfall_single_patient_streamlit(
                    shap_vals_AKI,
                    shap_data_AKI,
