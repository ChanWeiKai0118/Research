        ...
        if input_number and input_date_str:
            try:
                ...
                if selected_row.empty:
                    ...
                    st.markdown(f"### Predicted AKD Risk: n/ <span style='color:{get_akd_color(akd_prob)};font-weight:bold;'>{akd_prob:.4f}%</span> (dose at {dose_percentage}%)",unsafe_allow_html=True)
                    ...
                    st.markdown(f"### Predicted AKI Risk: n/ <span style='color:{get_aki_color(aki_prob)}; font-weight:bold;'>{aki_prob:.2f}%</span> (dose at {dose_percentage}%)",unsafe_allow_html=True)
